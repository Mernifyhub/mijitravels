...
const Toast = ({
  ...
  return (
    <div className={`fixed top-20 right-4 z-[200] flex items-center gap-3 px-5 py-3.5
      rounded-xl shadow-2xl text-white min-w-72 ${cfg.cls}`}>
      {cfg.icon}
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b bg-gray-50">
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => setZoom((z) => Math.max(0.5, z - 0.25))}
              className="p-2 hover:bg-gray-200 rounded-lg transition" title="Zoom out">
            ...
            <button onClick={() => setZoom((z) => Math.min(3, z + 0.25))}
              className="p-2 hover:bg-gray-200 rounded-lg transition" title="Zoom in">
              <ZoomIn size={16} className="text-gray-600" />
            </button>
            <div className="h-5 w-px bg-gray-200 mx-1" />
            <button onClick={() => setRotation((r) => (r + 90) % 360)}
              className="p-2 hover:bg-gray-200 rounded-lg transition" title="Rotate">
              <RotateCw size={16} className="text-gray-600" />
            </button>
            <a href={url} target="_blank" rel="noopener noreferrer"
              className="p-2 hover:bg-gray-200 rounded-lg transition" title="Open full size">
        ...
        <div className="px-5 py-3 border-t bg-gray-50 flex items-center justify-between">
          ...
          <a href={url} download
            className="flex items-center gap-2 px-4 py-2 bg-[#0B2545] text-white
              rounded-xl text-sm font-medium hover:bg-[#0a1f3a] transition">
...
const DetailModal = ({
  ...
  >
    ...
    >
      {/* Header */}
      <div className="bg-gradient-to-r from-[#0B2545] to-[#134074] p-5 text-white
      ...
      <div className="p-6 space-y-5">
            ...
            <div>
              ...
              {request.processedAt && (
                <p className="text-xs text-gray-500">
                  {fmtDateTime(request.processedAt)}
        ...
        <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100">
          ...
          <div className="flex items-start gap-4 mb-4">
            <div className="w-14 h-14 bg-gradient-to-br from-[#0B2545] to-[#134074]
              rounded-2xl flex items-center justify-center text-white text-xl font-bold shrink-0">
            ...
            <div className="flex-1">
              ...
              <span className="inline-block mt-1 px-3 py-0.5 bg-[#0B2545] text-white
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            ...
            <div className="bg-blue-100 rounded-xl p-3 border border-blue-200">
              ...
              <p className="font-bold text-blue-700 text-sm">
                {fmt(request.previousBalance + request.amount)}
              </p>
            </div>
          </div>
        </div>
        ...
        {request.attachment ? (
          <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100">
            ...
            <div onClick={onViewReceipt}
              className="relative group cursor-pointer rounded-xl overflow-hidden
                border-2 border-dashed border-slate-300 hover:border-blue-400 transition-all">
              <img src={request.attachment} alt="Receipt"
                className="w-full h-48 object-cover"
                onError={(e) => { (e.target as HTMLImageElement).src = "/placeholder-receipt.png"; }} />
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40
                transition-all flex items-center justify-center">
                <span className="opacity-0 group-hover:opacity-100 transition-all
                  flex items-center gap-2 bg-white text-gray-800 px-4 py-2
          ...
          <div className="bg-slate-50 rounded-2xl p-5 border border-dashed border-slate-300">
            ...
            <p className="text-sm text-gray-400 flex items-center gap-2">
              <Image size={16} /> No receipt uploaded
            </p>
          </div>
        )}
        ...
        <div className="p-5 border-t bg-gray-50 flex gap-3 sticky bottom-0 rounded-b-2xl">
          <button onClick={onReject}
            className="flex-1 py-3 bg-red-600 text-white rounded-xl hover:bg-red-700
              font-semibold transition flex items-center justify-center gap-2 shadow-sm">
    ...
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md"
      ...
      <div className="p-8">
        ...
        <div className="bg-gray-50 rounded-xl p-4 mb-4 flex items-center gap-3">
          <div className="w-10 h-10 bg-gradient-to-br from-[#0B2545] to-[#134074]
        ...
        <div className="flex gap-3">
          <button onClick={onClose} disabled={loading}
            className="flex-1 py-3 border border-gray-200 text-gray-700 rounded-xl
              hover:bg-gray-50 transition font-semibold disabled:opacity-50">
            Cancel
          </button>
          <button onClick={onConfirm} disabled={loading}
            className="flex-1 py-3 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700
              transition font-bold shadow-lg disabled:opacity-50
    ...
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md"
      ...
      <div className="p-8">
        ...
        <p className="text-center text-gray-400 text-sm mb-6">
          <span className="font-bold text-red-600">{fmt(request.amount)}</span>{" "}
          from <span className="font-semibold">{request.agentName}</span>
        </p>
        ...
        <div className="mb-6">
          ...
          <textarea
            ...
            className="w-full px-4 py-3 border border-gray-200 rounded-xl
              focus:outline-none focus:ring-2 focus:ring-red-400 resize-none text-sm"
            autoFocus
          />
...
export default function AdminDepositManagement() {
  ...
  const handleBulkApprove = async () => {
    const pendingIds = data
      .filter((d) => selectedItems.includes(d.id) && d.status === "PENDING")
      .map((d) => d.id);
  ...
  const filteredData = useMemo(() => {
    ...
    // ✅ Fixed sort — no more 'av is possibly undefined'
    if (sortConfig.key) {
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative group">
                <button className="flex items-center gap-1.5 px-3.5 py-2 border border-gray-200
                  text-gray-600 rounded-xl hover:bg-gray-50 text-sm font-medium transition">
                  <Download size={15} /> Export <ChevronDown size={13} />
                </button>
                <div className="absolute right-0 top-full mt-1.5 w-40 bg-white rounded-xl
                  shadow-xl border py-1.5 opacity-0 invisible group-hover:opacity-100
                  ...
                  {[
                    ...
                    <button key={e.f} onClick={() => handleExport(e.f as "csv" | "excel" | "pdf")}
              ...
              <button
                ...
                disabled={refreshing}
                className="flex items-center gap-1.5 px-3.5 py-2 border border-gray-200
                  text-gray-600 rounded-xl hover:bg-gray-50 text-sm font-medium
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="flex gap-2 flex-1">
                <div className="relative flex-1 max-w-sm">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-9 py-2.5 border border-gray-200 rounded-xl
                      text-sm focus:outline-none focus:ring-2 focus:ring-[#0B2545]/20
                      focus:border-[#0B2545] transition"
                  />
                  {searchTerm && (
                ...
                <button
                  onClick={() => {
                    setFilterType("all"); setFilterMethod("all");
                    setSearchTerm(""); setActiveTab("all"); setCurrentPage(1);
                  }}
                  className="px-3.5 py-2.5 bg-gray-100 text-gray-500 rounded-xl
                    hover:bg-gray-200 text-sm font-medium flex items-center gap-1.5 transition"
                >
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl border
                ...
                >
                  ...
                  {activeFiltersCount > 0 && (
                    <span className="w-4 h-4 bg-blue-600 text-white rounded-full text-[10px]
                      flex items-center justify-center">
                      {activeFiltersCount}
                    </span>
                  )}
                ...
                <select
                  ...
                  className="px-3 py-2.5 border border-gray-200 rounded-xl text-sm
                    text-gray-600 focus:outline-none focus:ring-2 focus:ring-[#0B2545]/20 bg-white"
                >
                  {[5, 10, 25, 50].map((n) => (
            ...
            {selectedItems.length > 0 && (
              <div className="mt-3 flex items-center gap-3 p-3 bg-blue-50 rounded-xl
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-100
                    text-emerald-700 rounded-lg hover:bg-emerald-200 text-sm
              ...
              <div className="mt-3 pt-3 border-t border-gray-100
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-400 mb-1.5
                    uppercase tracking-wide">Type</label>
                  <select value={filterType} onChange={(e) => setFilterType(e.target.value)}
                    className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm
                      focus:outline-none focus:ring-2 focus:ring-[#0B2545]/20 bg-white">
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-400 mb-1.5
                    uppercase tracking-wide">Method</label>
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-400 mb-1.5
                    uppercase tracking-wide">From Date</label>
                  <input type="date"
                    className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm
                      focus:outline-none focus:ring-2 focus:ring-[#0B2545]/20" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-400 mb-1.5
                    uppercase tracking-wide">To Date</label>
                  <input type="date"
                    className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm
                      focus:outline-none focus:ring-2 focus:ring-[#0B2545]/20" />
                </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                <tr className="bg-slate-50 border-b border-gray-100">
                  <th className="px-4 py-3.5 w-10">
                    <input
                      ...
                      className="w-4 h-4 rounded border-gray-300 text-[#0B2545]
                        focus:ring-[#0B2545]"
                    />
                  </th>
                  ...
                  {[
                    ...
                    <th
                      ...
                      className={`px-4 py-3.5 text-xs font-bold text-gray-500 uppercase
                        tracking-wider
                        ${col.key    ? "cursor-pointer hover:text-gray-800 transition select-none" : ""}
                        ${"center" in col && col.center ? "text-center" : "text-left"}`}
                    >
                      <div className={`flex items-center gap-1.5
                        ${"center" in col && col.center ? "justify-center" : ""}`}>
                        {col.label}
              ...
              <tbody className="divide-y divide-gray-50">
                {paginatedData.length === 0 ? (
                    ...
                    <td colSpan={9} className="py-20 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <button
                          onClick={() => {
                            setFilterType("all"); setFilterMethod("all");
                            setSearchTerm(""); setActiveTab("all");
                          }}
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 bg-gradient-to-br from-[#0B2545] to-[#134074]
                          rounded-xl flex items-center justify-center text-white font-bold
                          text-sm shrink-0">
                      ...
                      {item.attachment ? (
                        <button onClick={() => setReceiptItem(item)}
                          className="inline-flex items-center gap-1 px-3 py-1.5
                            bg-purple-50 text-purple-600 rounded-lg hover:bg-purple-100
                            text-xs font-semibold transition border border-purple-200">
                          <Image size={12} /> View
                        </button>
                      ) : (
          ...
          {filteredData.length > 0 && (
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between
              px-5 py-4 border-t border-gray-50 bg-gray-50/30">
