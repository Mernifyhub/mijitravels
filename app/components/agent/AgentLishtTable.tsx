...
import {Search, Users, Download, RefreshCw, ChevronLeft, ChevronRight,ChevronsLeft, ChevronsRight, ArrowUpDown, ArrowUp, ArrowDown,
Edit, Trash2, MoreHorizontal, CheckCircle2, XCircle, Clock,AlertCircle, X, ChevronDown, Mail, Phone, MapPin, Building2,Wallet, TrendingUp, UserCheck, UserX, UserPlus, 
Copy, Check,Ban, Unlock, Send, History, Star, Globe, Info, Loader2,SlidersHorizontal, RotateCcw, Sparkles, Award, Target, Printer,FileSpreadsheet, File, AlertTriangle,
Power, PowerOff, WifiOff,ServerCrash, DatabaseZap, Eye, Receipt,} from "lucide-react";
...
// ==================== API SERVICE ====================
// ✅ NestJS backend — uses apiClient pattern
...
const apiFetch = async (endpoint: string, options: RequestInit = {}) => {
  ...
  if (!res.ok) {
    ...
    throw {
      message: err.message || err.error || `HTTP Error: ${res.status}`,
...
class AgentApiService {
  async getAgents(params: {
    ...
    Object.entries(params).forEach(([k, v]) => {
      if (v !== undefined && v !== null && v !== "") {
        // ✅ pageSize → limit for NestJS
        const key = k === "pageSize" ? "limit" : k;
    ...
    if (raw?.agents && Array.isArray(raw.agents)) {
      ...
      totalPages =
        raw.totalPages || Math.ceil(total / (params.pageSize || 10));
      stats = raw.stats;
  ...
  async createAgent(data: Partial<Agent>): Promise<Agent> {
    const raw = await apiFetch("/admin/agents", {
      method: "POST",
  ...
  async updateAgent(internalId: string, data: Partial<Agent>): Promise<Agent> {
    const raw = await apiFetch(`/admin/agents/${internalId}`, {
      method: "PUT",
      body: JSON.stringify(data),
    });
    return normalizeAgent(raw.agent || raw);
  ...
  async togglePreBooking(internalId: string, enabled: boolean): Promise<Agent> {
    const raw = await apiFetch(`/admin/agents/${internalId}/pre-booking`, {
      method: "PATCH",
      body: JSON.stringify({ preBookingEnabled: enabled }),
    });
    return normalizeAgent(raw.agent || raw);
  }
  ...
  async bulkAction(
    ...
    const raw = await apiFetch("/admin/agents/bulk", {
      method: "POST",
      body: JSON.stringify({ ids: internalIds, action }),
    });
    return raw;
  }
}
...
function useAgents(initialStatus = "") {
  ...
  const [stats, setStats] = useState({
    ...
    suspended: 0, inactive: 0, totalBalance: 0, totalRevenue: 0,
  });
  ...
  const getInternalId = useCallback(
    (displayId: string): string => {
      ...
      return agent?.internalId || displayId;
    },
    [agents],
  );
  ...
  const updateAgent = async (displayId: string, data: Partial<Agent>) => {
    ...
    try {
      ...
      setAgents((prev) =>
        prev.map((a) =>
          a.id === displayId
            ? { ...a, ...updated, id: a.id, internalId: a.internalId }
            : a,
        ),
      );
      return updated;
  ...
  setAgents((prev) =>
    prev.map((a) =>
      a.id === displayId
        ? { ...a, status: status.toLowerCase() as Agent["status"] }
        : a,
    ),
  );

  try {
    await agentApi.updateAgentStatus(internalId, status);
    await fetchAgents(); // ✅ re-fetch
  } catch (err) {
  ...
  setAgents((p) =>
    p.map((a) =>
      a.id === displayId ? { ...a, preBookingEnabled: enabled } : a,
    ),
  );

  try {
    await agentApi.togglePreBooking(internalId, enabled);
    await fetchAgents(); // ✅ re-fetch
  } catch (err) {
    // Rollback
    setAgents((p) =>
  ...
  const bulkAction = async (displayIds: string[], action: string) => {
    ...
    try {
      await agentApi.bulkAction(internalIds, action);
      await fetchAgents();
    } finally {
      setActionLoading(null);
    }
...
const getTierConfig = (tier: string) => {
  const configs: Record<string, { bg: string; text: string; icon: JSX.Element; gradient: string }> = {
      ...
      >
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button
            ...
            className={`flex-1 py-2.5 text-white rounded-xl font-semibold transition disabled:opacity-50 flex items-center justify-center gap-2 ${confirmClass}`}
          >
...
const ErrorState = ({ error, onRetry }: { error: ApiError; onRetry: () => void }) => (
  ...
  >
    <div className="flex flex-col items-center">
      ...
      <button
        onClick={onRetry}
        className="px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl font-semibold hover:from-blue-700 hover:to-indigo-700 transition flex items-center gap-2"
      >
...
const ToggleSwitch = ({ enabled, onChange, loading }: {
  enabled: boolean; onChange: () => void; loading?: boolean;
}) => (
  <button
    onClick={(e) => { e.preventDefault(); e.stopPropagation(); if (!loading) onChange(); }}
    disabled={loading}
    type="button"
    className={`relative inline-flex items-center w-9 h-5 rounded-full transition-colors duration-200 focus:outline-none disabled:cursor-not-allowed ${
  ...
  >
    {loading ? (
      ...
      <span className={`absolute top-[3px] w-3.5 h-3.5 bg-white rounded-full shadow transition-all duration-200 ${
...
const ActionDropdown = ({
  ...
  agent: Agent; onView: () => void; onEdit: () => void;
  onEnable: () => void; onDisable: () => void; onDelete: () => void; loading?: boolean;
}) => {
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute right-0 top-full mt-1 w-48 bg-white rounded-xl shadow-xl border border-slate-100 py-1.5 z-30"
          >
            {[
            ...
            {agent.status !== "active" ? (
              <button
                onClick={() => { onEnable(); setIsOpen(false); }}
              ...
              <button
                onClick={() => { onDisable(); setIsOpen(false); }}
                className="w-full px-4 py-2 text-sm text-left text-amber-600 hover:bg-amber-50 flex items-center gap-2"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {[
                ...
                <div key={i} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl">
                  ...
                  {item.copyable && (
                    <button onClick={() => handleCopy(item.val)} className="p-1.5 hover:bg-slate-200 rounded-lg transition">
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between bg-slate-50">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-4 py-2 bg-slate-200 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-300 transition flex items-center gap-2"
            >
              <Edit size={16} /> Edit
            </button>
            {agent.status === "active" ? (
              <button
                ...
                className="px-4 py-2 bg-rose-500 text-white rounded-lg text-sm font-medium hover:bg-rose-600 transition flex items-center gap-2 disabled:opacity-50"
              >
              ...
              <button
                ...
                disabled={statusLoading}
                className="px-4 py-2 bg-emerald-500 text-white rounded-lg text-sm font-medium hover:bg-emerald-600 transition flex items-center gap-2 disabled:opacity-50"
              >
...
const AgentFormModal = ({
  ...
  agent: Agent | null; isOpen: boolean; onClose: () => void;
  onSubmit: (data: Partial<Agent>) => void; loading: boolean;
}) => {
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-5 text-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold">{isEdit ? "Edit Agent" : "Add New Agent"}</h2>
                <p className="text-blue-100 text-sm">{isEdit ? "Update agent information" : "Create a new agent account"}</p>
              </div>
            </div>
            ...
            <div className="md:col-span-2">
              ...
              <div
                ...
                className={`flex items-center gap-3 p-4 border-2 rounded-xl cursor-pointer transition ${
                  formData.preBookingEnabled ? "border-emerald-400 bg-emerald-50" : "border-slate-200 bg-slate-50"
                }`}
              >
...
const ExportDropdown = ({ onExport, loading }: { onExport: (format: string) => void; loading?: boolean }) => {
  const [isOpen, setIsOpen] = useState(false);
  return (
    <div className="relative">
      <button
        onClick={() => setIsOpen(!isOpen)} disabled={loading}
        className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 transition shadow-sm disabled:opacity-50"
      >
...
export default function AgentManagement({ defaultStatus = "" }: { defaultStatus?: string }) {
  ...
  const handleEditAgent = (agent: Agent) => { setEditingAgent(agent); setShowDetailModal(false); setShowFormModal(true); };
  ...
  const activeFilterCount = [statusFilter, tierFilter, countryFilter].filter(
    (f) => f && f !== defaultStatus,
  ).length;
      ...
      <div className="max-w-[1800px] mx-auto space-y-5">
        ...
        >
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-slate-800 flex items-center gap-3">
              ...
              Agent Management
              {defaultStatus && (
                <span className={`text-sm px-3 py-1 rounded-full font-medium ${getStatusConfig(defaultStatus).bg} ${getStatusConfig(defaultStatus).text}`}>
                  {getStatusConfig(defaultStatus).label}
                </span>
              )}
            </h1>
            <p className="text-slate-500 mt-1">
              {defaultStatus ? `Showing ${defaultStatus} agents only` : "Manage all travel agents and their accounts"}
            </p>
          </div>
          <div className="flex items-center gap-2 md:gap-3 flex-wrap">
            <ExportDropdown onExport={handleExport} loading={false} />
            <button
              onClick={() => fetchAgents()} disabled={loading}
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl text-sm font-medium hover:from-blue-700 hover:to-indigo-700 transition shadow-sm"
            >
          ...
          >
            {[
              ...
              <div key={idx} className={`rounded-2xl p-4 border shadow-sm ${
                stat.isDark ? "bg-gradient-to-br from-slate-800 to-slate-900 border-slate-700" : `bg-white ${stat.border}`
              }`}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className={`text-xs ${stat.isDark ? "text-slate-400" : "text-slate-500"}`}>{stat.label}</p>
                    <p className={`font-bold ${stat.isDark ? "text-white" : stat.text} ${stat.isLarge ? "text-base mt-0.5" : "text-2xl"}`}>
                      {stat.value}
                    </p>
                  </div>
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                    stat.isDark ? "bg-white/10 text-emerald-400" : `${stat.bg} ${stat.text}`
                  }`}>
                    {stat.icon}
                  </div>
                </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-3">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input
                ...
                className="w-full pl-11 pr-10 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition"
              />
              {searchQuery && (
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {!defaultStatus && (
                <div className="relative">
                  <select
                    ...
                    className="appearance-none pl-4 pr-10 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none bg-white cursor-pointer min-w-[120px]"
                  >
              ...
              <button
                ...
                className={`px-4 py-2.5 rounded-xl text-sm font-medium flex items-center gap-2 transition relative ${
              ...
              <button
                onClick={handleReset}
                className="px-4 py-2.5 bg-slate-100 text-slate-700 rounded-xl text-sm font-medium hover:bg-slate-200 transition flex items-center gap-2"
              >
                <RotateCcw size={15} />
              ...
              >
                <div className="mt-3 pt-3 border-t border-slate-100 grid grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <div key={label}>
                      ...
                      <div className="relative">
                        <select
                          ...
                          className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm bg-white appearance-none pr-10 focus:outline-none"
                        >
                  ...
                  <div className="col-span-2 flex items-end">
                    <button onClick={handleReset} className="px-4 py-2 text-sm text-rose-500 hover:text-rose-700 font-medium transition">
                      Clear all filters
                    </button>
                  </div>
                </div>
              ...
              <div className="flex items-center gap-2">
                {[
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 text-white rounded-lg text-xs font-medium transition disabled:opacity-50 ${cls}`}
                  >
                    {icon} {label}
                  </button>
                ))}
                <button onClick={() => setSelectedIds(new Set())} className="p-1.5 bg-white/10 text-white rounded-lg hover:bg-white/20 transition">
                  <X size={16} />
                </button>
              </div>
            </motion.div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
          <p className="text-sm text-slate-500">
            Showing <span className="font-semibold text-slate-700">{agents.length}</span> of{" "}
          ...
          <div className="flex items-center gap-2 text-sm text-slate-500">
            ...
            <div className="relative">
              <select
                ...
                className="border border-slate-200 rounded-lg px-3 py-1.5 text-sm bg-white appearance-none pr-8 cursor-pointer focus:outline-none"
              >
              ...
              >
                {loading && agents.length > 0 && (
                  <div className="absolute inset-0 bg-white/60 backdrop-blur-[1px] z-10 flex items-center justify-center">
                    <div className="flex items-center gap-3 bg-white px-5 py-3 rounded-xl shadow-lg border border-slate-100">
                      ...
                      <tr className="bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200">
                        ...
                        <th className="px-3 py-3.5 text-right">
                          <button
                            ...
                            className="flex items-center gap-1 text-xs font-semibold text-slate-500 uppercase tracking-wider hover:text-slate-800 transition ml-auto"
                          >
                        ...
                        <th className="px-3 py-3.5 text-right">
                          <button
                            ...
                            className="flex items-center gap-1 text-xs font-semibold text-slate-500 uppercase tracking-wider hover:text-slate-800 transition ml-auto"
                          >
                            Credit Limit <SortIcon column="creditLimit" />
                          </button>
                        </th>
                        <th className="px-3 py-3.5 text-center">
                          <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Staffs</span>
                        </th>
                        <th className="px-3 py-3.5 text-center">
                          <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Pre Booking</span>
                        </th>
                        <th className="px-3 py-3.5 text-center">
                        ...
                        agents.map((agent) => {
                          ...
                          return (
                            <motion.tr
                              ...
                              className={`hover:bg-slate-50/80 transition group ${isSelected ? "bg-blue-50/60" : ""} ${isThisLoading ? "opacity-60" : ""}`}
                            >
                              ...
                              <td className="px-3 py-3">
                                <div className="flex items-center gap-1">
                                  <Link
                                    href={`/admin/agent/${agent.internalId}/ledger`}
                                    className="px-2 py-0.5 bg-slate-100 text-blue-600 hover:text-blue-800 hover:bg-blue-50 rounded text-xs font-mono font-medium transition"
                                  >
                                    {agent.id}
                                  </Link>
                                  {agent.verified && (
                              ...
                              <td className="px-3 py-3">
                                <div className="flex items-center gap-2.5">
                                  <div className={`w-9 h-9 bg-gradient-to-br ${tc.gradient} rounded-xl flex items-center justify-center text-white font-bold text-sm shrink-0`}>
                                    {(agent.agentName || agent.name)?.charAt(0)}
                                  </div>
                                  <div className="min-w-0">
                                    <p className="font-semibold text-slate-800 text-sm leading-tight truncate max-w-[130px]">
                                      {agent.agentName || agent.name}
                                    </p>
                                    <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-semibold mt-0.5 ${sc.bg} ${sc.text}`}>
                                      {sc.icon} {sc.label}
                                    </span>
                                  </div>
                                </div>
                              ...
                              <td className="px-3 py-3">
                                <div className="flex items-center gap-1.5">
                                  ...
                                  <span className="text-xs text-slate-600 truncate max-w-[160px]">{agent.email}</span>
                                </div>
                              </td>
                              ...
                              <td className="px-3 py-3">
                                <div className="flex items-center gap-1.5">
                                  ...
                                  <span className="text-xs text-slate-600 whitespace-nowrap">{agent.phone || "—"}</span>
                                </div>
                              </td>
                              ...
                              <td className="px-3 py-3">
                                <div className="flex items-center gap-1.5">
                                  ...
                                  <span className="text-xs text-slate-600 whitespace-nowrap">{agent.country || "—"}</span>
                                </div>
                              </td>

                              {/* Balance */}
                              <td className="px-3 py-3 text-right">
                                <p className="font-bold text-slate-800 text-sm">{formatCurrency(agent.balance)}</p>
                                <p className="text-[10px] text-slate-400">{agent.totalBookings} bookings</p>
                              </td>

                              {/* Credit Limit */}
                              <td className="px-3 py-3 text-right">
                                <p className="font-bold text-slate-800 text-sm">{formatCurrency(agent.creditLimit)}</p>
                              ...
                              <td className="px-3 py-3 text-center">
                                <div className="flex flex-col items-center gap-1">
                                  ...
                                  />
                                  <span className={`text-[10px] font-medium ${agent.preBookingEnabled ? "text-emerald-600" : "text-slate-400"}`}>
                                ...
                                <div className="flex items-center justify-center gap-1">
                                  {agent.status === "active" ? (
                                    <button
                                      ...
                                      className="p-1.5 hover:bg-amber-50 rounded-lg transition group/btn disabled:opacity-50"
                                      title="Disable"
                                    >
                                      {isStatusLoading
                                    ...
                                    <button
                                      ...
                                      disabled={isThisLoading}
                                      className="p-1.5 hover:bg-emerald-50 rounded-lg transition group/btn disabled:opacity-50"
                                      title="Enable"
                                    >
                                      {isStatusLoading
                                  ...
                                  <button
                                    ...
                                    disabled={isThisLoading}
                                    className="p-1.5 hover:bg-blue-50 rounded-lg transition group/btn disabled:opacity-50"
                                    title="Edit"
                                  >
                                  ...
                                  <button
                                    ...
                                    disabled={isThisLoading}
                                    className="p-1.5 hover:bg-rose-50 rounded-lg transition group/btn disabled:opacity-50"
                                    title="Delete"
                                  >
                                    {actionLoading === `delete-${agent.id}`
                        ...
                        <tr>
                          <td colSpan={11} className="px-4 py-16 text-center">
                            ...
                            >
                              ...
                              <button
                                onClick={handleReset}
                                className="mt-4 px-5 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 transition flex items-center gap-2"
                              >
      ...
      <AnimatePresence>
        {showFormModal && (
          <AgentFormModal
            ...
            loading={actionLoading === "create" || !!(editingAgent && actionLoading === `update-${editingAgent.id}`)}
          />
        )}
      </AnimatePresence>
