...
// ==================== TYPES ====================
interface Agent {
  id: string;          // UUID - backend এর জন্য
  internalId: string;  // MPA001 - UI display এর জন্য
  name: string;
  email: string;
  phone: string;
  company: string;
...
function validateOperationForm(
  ...
  switch (operationId) {
    ...
    case "date_change": {
      ...
      if (!formData.amount || Number(formData.amount) === 0)
        return "Amount is required (negative for refund)";
      return null;
    }
    case "add_credit": {
      if (!formData.amount || Number(formData.amount) <= 0)
      ...
      if (!formData.description?.trim())
        return "Reason is required for credit addition";
      return null;
    }
    case "adm":
    case "acm": {
    ...
    case "limit_add": {
      if (!formData.newLimit || Number(formData.newLimit) < 0)
        return "New credit limit is required";
...
// ✅ buildPayload - formData (raw strings) থেকে API payload বানায়
function buildPayload(
  operationId: string,
  formData: Record<string, string>,
  agent: Agent
): Record<string, any> {
  // ✅ agent.id = UUID (backend এর user.id)
  ...
  switch (operationId) {
    case "manual_booking":
      return {
        ...
        route: formData.route.trim().toUpperCase(),
        travelDate: formData.travelDate,
        description:
          formData.description?.trim() ||
          `Manual booking - ${formData.pnr.trim().toUpperCase()}`,
      };
    ...
    case "date_change":
      return {
        ...
        description:
          formData.description?.trim() ||
          `Date change - ${formData.pnr.trim().toUpperCase()}`,
      };
    ...
    case "add_credit":
      return {
        ...base,
        amount: Number(formData.amount),
        newLimit: agent.creditLimit + Number(formData.amount),
        previousLimit: agent.creditLimit,
        description: formData.description.trim(),
        reference: formData.reference?.trim() || undefined,
      };
    ...
    case "limit_add":
      return {
        ...base,
        newLimit: Number(formData.newLimit),
        previousLimit: agent.creditLimit,
        amount: Math.abs(
          Number(formData.newLimit) - agent.creditLimit
        ),
        description: formData.description.trim(),
        reference: formData.reference?.trim() || undefined,
      };
...
function useAgentList() {
  ...
  const fetchAgents = useCallback(async () => {
    setLoading(true);
    try {
      // ✅ NestJS: GET /api/v1/admin/agents
      ...
      const normalized: Agent[] = list.map((a: any) => ({
        ...
        totalBookings: Number(
          a._count?.bookings || a.totalBookings || 0
        ),
...
function useOperations() {
  ...
  const fetchOperations = useCallback(async () => {
    setLoading(true);
    try {
      // ✅ NestJS: GET /api/v1/admin/operations
  ...
  const submitOperation = async (payload: any): Promise<any> => {
    setSubmitLoading(true);
    try {
      // ✅ NestJS: POST /api/v1/admin/operations
      const result = await apiClient("/admin/operations", {
        method: "POST",
        body: JSON.stringify(payload),
      });

      // ✅ Submit হলে list refresh করো
      await fetchOperations();

      return result;
    } catch (err: any) {
      // ✅ Error properly throw করো
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className="w-full flex items-center justify-between px-4 py-3.5 bg-white border border-slate-200 rounded-xl hover:border-slate-300 transition"
      >
        {selectedAgent ? (
...
const OperationTypeCard = ({
  ...
  >
    ...
    <div
      className={`w-14 h-14 ${operation.bgColor} rounded-xl flex items-center justify-center ${operation.color} mb-4`}
    >
      {operation.icon}
    </div>
    ...
    {isSelected && (
      <motion.div
        initial={{ scale: 0 }}
        animate={{ scale: 1 }}
        className={`absolute bottom-3 right-3 w-6 h-6 bg-gradient-to-r ${operation.gradient} rounded-full flex items-center justify-center`}
      >
...
const AgentInfoCard = ({ agent }: { agent: Agent }) => {
  ...
  const pct =
    agent.creditLimit > 0
      ? (agent.usedLimit / agent.creditLimit) * 100
      : 0;
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              {/* ✅ internalId (MPA001) দেখাও, UUID নয় */}
        ...
        <div className="bg-slate-50 rounded-xl p-4">
          ...
          <div className="flex justify-between text-xs">
            <span className="text-slate-500">
              Used: {fmt(agent.usedLimit)}
            </span>
            <span className="text-slate-500">
              Limit: {fmt(agent.creditLimit)}
            </span>
          </div>
...
const OperationForm = ({
  ...
  const [formData, setFormData] =
    useState<Record<string, string>>(emptyForm);
  ...
  // Operation টাইপ change হলে form reset হবে
  useEffect(() => {
    resetForm();
  }, [selectedOperation?.id]); // eslint-disable-line react-hooks/exhaustive-deps

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedAgent || !selectedOperation) return;

    const error = validateOperationForm(
    ...
    setFormError(null);
    // ✅ raw formData পাঠাও, buildPayload main এ হবে
    onSubmit(formData);
  };
  ...
  const inp =
    "w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 bg-white";
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  value={formData.pnr}
                  onChange={(e) =>
                    set("pnr", e.target.value.toUpperCase())
                  }
                  placeholder="ABC123"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    set("route", e.target.value.toUpperCase())
                  }
                  placeholder="RUH - DAC"
                  className={`${inp} uppercase`}
                />
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="0.00"
                  className={`${inp} pl-14 text-lg font-semibold`}
                  min="0"
                  step="0.01"
                />
              </div>
          ...
          <>
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">
                Negative value for refund (e.g., -500)
              </p>
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">
        ...
        return (
          <>
            ...
            <div>
              ...
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-teal-600 font-bold">
                  +SAR
                </span>
                <input
                  type="number"
                  ...
                  onChange={(e) => set("amount", e.target.value)}
                  placeholder="0"
                  className={`${inp} pl-16 text-2xl font-bold`}
                  min="0"
                  step="1"
                />
              </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-4 gap-2">
                {[5000, 10000, 15000, 20000, 25000, 50000, 75000, 100000].map(
                  (v) => (
                    <button
                      key={v}
                      type="button"
                      onClick={() => set("amount", v.toString())}
            ...
            <div>
              ...
              <textarea
                value={formData.description}
                onChange={(e) => set("description", e.target.value)}
                placeholder="Emergency credit reason..."
                rows={3}
                className={`${inp} resize-none`}
              />
            </div>
          ...
          <>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => set("reference", e.target.value)}
                placeholder={
                  selectedOperation.id === "adm" ? "ADM-123" : "ACM-123"
                }
                className={`${inp} font-mono`}
              />
            </div>
          ...
          <>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`${inp} pl-14 text-2xl font-bold`}
                  min="0"
                  step="1000"
                />
              </div>
            ...
            <div>
              ...
              <textarea
                value={formData.description}
                onChange={(e) => set("description", e.target.value)}
                placeholder="Reason for limit adjustment..."
                rows={3}
                className={`${inp} resize-none`}
              />
            </div>
        ...
        return (
          <>
            <div>
              ...
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-rose-500 font-medium">
                  -SAR
                </span>
                <input
                  type="number"
  ...
  return (
    ...
    >
      <div
        className={`bg-gradient-to-r ${selectedOperation.gradient} p-5 text-white`}
      >
        <div className="flex items-center gap-4">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
        ...
        <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-slate-500">
              {selectedAgent.internalId} • Balance: SAR{" "}
              {selectedAgent.balance.toLocaleString()} • Limit: SAR{" "}
              {selectedAgent.creditLimit.toLocaleString()}
            </p>
          </div>
          {/* Copy UUID for internal use */}
          <button
            type="button"
            onClick={() => handleCopy(selectedAgent.id)}
        ...
        <div className="flex items-center gap-3 pt-4 border-t border-slate-100">
          <button
            type="button"
            onClick={resetForm}
            className="flex-1 py-3 bg-slate-100 text-slate-700 rounded-xl font-semibold hover:bg-slate-200 transition flex items-center justify-center gap-2"
          >
          ...
          <button
            type="submit"
            disabled={loading}
            className={`flex-1 py-3 bg-gradient-to-r ${selectedOperation.gradient} text-white rounded-xl font-semibold hover:opacity-90 transition disabled:opacity-50 flex items-center justify-center gap-2`}
          >
            {loading ? (
              <>
...
const ConfirmationModal = ({
  ...
  if (operation.id === "add_credit" && formData.amount) {
    rows.push({
      label: "Credit to Add",
      value: `+SAR ${Number(formData.amount).toLocaleString()}`,
      highlight: "text-emerald-600",
    });
    rows.push({
      label: "New Credit Limit",
      value: `SAR ${(
        agent.creditLimit + Number(formData.amount)
      ).toLocaleString()}`,
    ...
    rows.push({
      label: "New Limit",
      value: `SAR ${Number(formData.newLimit).toLocaleString()}`,
  ...
  if (formData.passengerName)
    rows.push({ label: "Passenger", value: formData.passengerName });
  if (formData.route)
    rows.push({ label: "Route", value: formData.route });
  if (formData.travelDate)
    rows.push({ label: "Travel Date", value: formData.travelDate });
  if (formData.reference)
    rows.push({ label: "Reference", value: formData.reference });
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`bg-gradient-to-r ${operation.gradient} p-6 text-white text-center`}
        >
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex items-center gap-3 bg-slate-50">
          <button
            onClick={onClose}
            disabled={loading}
            className="flex-1 py-2.5 bg-slate-200 text-slate-700 rounded-xl font-medium hover:bg-slate-300 transition disabled:opacity-50"
          >
            Cancel
          </button>
          <button
            onClick={onConfirm}
            disabled={loading}
            className={`flex-1 py-2.5 bg-gradient-to-r ${operation.gradient} text-white rounded-xl font-medium hover:opacity-90 transition disabled:opacity-50 flex items-center justify-center gap-2`}
          >
            {loading ? (
              <>
...
const RecentOperations = ({
  ...
  const getOpType = (type: string) =>
    operationTypes.find((op) => op.id === type);
      ...
      <div className="divide-y divide-slate-100 max-h-96 overflow-y-auto">
        {loading ? (
          ...
          operations.map((op) => {
            ...
            const sc = getStatusCfg(op.status);
            return (
                  ...
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    <div
                      className={`w-10 h-10 shrink-0 ${
                        opType?.bgColor || "bg-slate-50"
                      } rounded-lg flex items-center justify-center ${
                        opType?.color || "text-slate-500"
                      }`}
                    >
...
export default function ManualOperationsPage() {
  ...
  // ✅ confirmationData = raw formData (string values)
  const [confirmationFormData, setConfirmationFormData] = useState<Record<
    string,
    string
  > | null>(null);
  ...
  // ✅ form submit হলে confirmation modal খোলে, formData save করে
  const handleFormSubmit = (formData: Record<string, string>) => {
    setConfirmationFormData(formData);
    setShowConfirmation(true);
  };

  // ✅ confirm করলে buildPayload চালায় এবং API call করে
  const handleConfirm = async () => {
    if (!selectedAgent || !selectedOperation || !confirmationFormData)
      return;
  ...
  // Agent list refresh হলে selected agent update করো
  useEffect(() => {
      ...
      <div className="max-w-[1800px] mx-auto space-y-6">
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <RefreshCw
                size={16}
                className={opsLoading ? "animate-spin" : ""}
              />{" "}
              Refresh
            </button>
            <button className="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-medium text-slate-700 hover:bg-slate-50 transition shadow-sm">
        ...
        >
          {[
            ...
            {
              label: "Total Credits",
              value: `SAR ${stats.totalCredit.toLocaleString()}`,
            ...
            {
              label: "Total Debits",
              value: `SAR ${stats.totalDebit.toLocaleString()}`,
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500">
                Choose the agent for this operation
              </p>
            </div>
            {selectedAgent && (
              <button
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-slate-400">
              {operationTypes.length} available
            </span>
          </div>
          ...
          >
            ...
            {selectedOperation && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-5 border border-blue-100"
              >
                      ...
                      {selectedOperation.id === "adm" && (
                        <li className="flex items-center gap-2">
                          <CheckCircle2
                            ...
                            className="text-emerald-500 shrink-0"
                          />
                          ADM deducts from agent balance — include airline
                          reference
                        </li>
                      )}
                      {selectedOperation.id === "acm" && (
                        <li className="flex items-center gap-2">
                          <CheckCircle2
                            ...
                            className="text-emerald-500 shrink-0"
                          />
                          ACM credits to agent balance — include airline
                          reference
                        </li>
                      )}
