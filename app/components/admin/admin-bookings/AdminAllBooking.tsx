...
import {
  ...
  Plane, Calendar, Users, TrendingUp, Clock, CheckCircle2,
  XCircle, AlertCircle, ArrowUpDown, ArrowUp, ArrowDown, SlidersHorizontal,
  FileText, RefreshCw, ChevronDown, X, Grid3X3, LayoutList, Settings2,
  UserCheck, Hash, CreditCard, Building2,
} from "lucide-react";
...
export default function AdminAllBooking({ defaultStatus = "" }: Props) {
  const router = useRouter();

  // ── SWR — admin endpoint ──
  const { data, error, isLoading: loading, isValidating: refreshing, mutate } =
    useSWR<Booking[]>("/admin/bookings", apiClient, {
  ...
  // ── Stats — includes unique agents count ──
  const stats = useMemo(() => {
    ...
    const uniqueAgents = new Set(bookings.map((b) => b.agent?.agentName).filter(Boolean)).size;
    return { total, confirmed, onHold, cancelled, totalRevenue, avgBookingValue, totalPassengers, uniqueAgents };
  }, [bookings]);

  // ── Filter + Sort — includes agent name search ──
  const filteredData = useMemo(() => {
    ...
    let result = bookings.filter((item) => {
      const matchSearch = query === "" ? true
        ...
        || item.agent?.agentName?.toLowerCase().includes(query)
        || item.agent?.agentId?.toLowerCase().includes(query)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-slate-100 p-4 md:p-6">
      <div className="max-w-[1800px] mx-auto space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            ...
            <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
              ...
              {stats.total} bookings • {stats.uniqueAgents} agents • {stats.totalPassengers} passengers • Avg {formatCurrency(stats.avgBookingValue)}/booking
            </motion.p>
          </div>
        ...
        {/* ── STATS — 7 cards including agents ── */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          {/* ✅ Agents card — admin only */}
          <div className="bg-white rounded-2xl p-4 border border-indigo-100 shadow-sm hover:shadow-md transition group">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-slate-500 font-medium">Agents</p>
                <p className="text-xl font-bold text-indigo-600 mt-1">{stats.uniqueAgents}</p>
              </div>
              <div className="w-10 h-10 bg-indigo-50 rounded-xl flex items-center justify-center group-hover:scale-110 transition">
            ...
            <div className="flex flex-1 gap-2 md:gap-3 flex-wrap md:flex-nowrap">
              <div className="relative flex-1 min-w-[200px]">
                ...
                <input ref={searchInputRef} value={searchInput} onChange={(e) => setSearchInput(e.target.value)}
                  placeholder="Search ID, PNR, Passenger, Route, Agent, Issued By..."
  ...
  <thead>
    <tr className="bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-100">
      ...
      {visibleColumns.includes("pnr") && (
        <th className="text-left px-3 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">PNR</th>
      )}
      {visibleColumns.includes("carrier") && (
        <th className="text-left px-3 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Carrier</th>
      )}
      {visibleColumns.includes("route") && (
        <th className="text-left px-3 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Route</th>
      )}
      ...
      {visibleColumns.includes("passenger") && (
        <th className="text-left px-3 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Passenger</th>
      )}
      ...
      {visibleColumns.includes("agent") && (
        <th className="text-left px-3 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Agent</th>
      )}
      {visibleColumns.includes("issuedBy") && (
        <th className="text-left px-3 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider">Issued By</th>
      )}
        ...
        <motion.tr key={b.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          {visibleColumns.includes("bookingId") && (
            <td className="px-3 py-3">
              <span className="inline-flex items-center gap-1.5 bg-slate-800 text-white px-2.5 py-1 rounded-lg text-xs font-semibold whitespace-nowrap">
          ...
          {visibleColumns.includes("status") && (
            <td className="px-3 py-3">
              <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold shadow-sm whitespace-nowrap ${sc.bg} ${sc.text} ${sc.glow}`}>
                {sc.icon}{sc.label}
              </span>
            </td>
          )}
          ...
          {visibleColumns.includes("carrier") && (
            <td className="px-3 py-3">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-sm font-medium text-slate-700 whitespace-nowrap">{b.carrier || "-"}</span>
              </div>
            </td>
          )}
          ...
          {visibleColumns.includes("departure") && (
            <td className="px-3 py-3">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-sm text-slate-700 whitespace-nowrap">{formatDate(b.departureDate)}</span>
              </div>
            </td>
          )}
          ...
          {visibleColumns.includes("bookingDate") && (
            <td className="px-3 py-3">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-sm text-slate-700 whitespace-nowrap">{formatDate(b.bookingDate)}</span>
              </div>
            </td>
          )}
          ...
          {visibleColumns.includes("agent") && (
            <td className="px-3 py-3">
              <div className="flex items-center gap-1.5">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-medium text-slate-700 truncate">{b.agent?.agentName || "-"}</p>
                  {b.agent?.agentId && (
                    <p className="text-xs text-slate-400 truncate">{b.agent.agentId}</p>
                  )}
                </div>
              </div>
            </td>
          ...
          {visibleColumns.includes("issuedBy") && (
            <td className="px-3 py-3">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-sm font-medium text-slate-700 truncate">{b.issuedBy || "-"}</span>
              </div>
            </td>
          )}
          ...
          {visibleColumns.includes("amount") && (
            <td className="px-3 py-3 text-right">
              <p className="text-sm font-bold text-slate-800 whitespace-nowrap">{formatCurrency(b.gross)}</p>
              <p className="text-xs text-slate-400 whitespace-nowrap">Net: {formatCurrency(b.net)}</p>
            </td>
          )}
        </motion.tr>
      );
                      ...
                      return (
                        <motion.div key={b.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                          ...
                          <div className="space-y-2 text-sm">
                            ...
                            {/* ✅ Agent in grid — admin only */}
                            <div className="flex items-center justify-between">
                              <span className="text-slate-500">Agent</span>
                              <span className="font-medium text-indigo-600">{b.agent?.agentName || "-"}</span>
                            </div>
                            <div className="flex items-center justify-between">
