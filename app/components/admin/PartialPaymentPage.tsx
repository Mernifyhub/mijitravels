...
import {
  Search, Users, CreditCard, Calendar, Clock,
  ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight,
  ArrowUpDown, ArrowUp, ArrowDown, Eye, Download, RefreshCw,
  X, ChevronDown, RotateCcw, SlidersHorizontal,
  TrendingUp, TrendingDown, Loader2, Info, CheckCircle2,
  XCircle, AlertCircle, AlertTriangle, History, Building2,
  Wallet, Banknote, Receipt, FileText, FileSpreadsheet, File,
  Printer, MoreHorizontal, Copy, Check, ExternalLink,
  Mail, Send, Plus, PlusCircle, MinusCircle, CircleDollarSign,
  PiggyBank, Timer, CalendarClock, CalendarCheck, CalendarX,
  DollarSign, Percent, Target, Sparkles, Bell, BellRing,
  MessageSquare, Phone, Hash, Verified, BadgePercent,
  ArrowRightLeft, Landmark, HandCoins, Coins, CircleCheck,
  CircleX, CircleDashed, ListChecks, Split, Layers, Edit,
  Trash2, Settings, HelpCircle, Shield, Plane,
} from "lucide-react";
...
const generatePartialPayments = (): PartialPayment[] => {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    for (let j = 0; j < installmentCount; j++) {
      ...
      const installmentStatus: PaymentInstallment["status"] = isPaid ? "paid" : isOverdue ? "overdue" : "pending";
      const paid = isPaid ? installmentAmount : 0;
      paidTotal += paid;
      installments.push({
        id:            `INS${i}${j}`,
...
const formatCurrency = (amount: number, currency = "SAR") =>
  new Intl.NumberFormat("en-SA", { style: "currency", currency, minimumFractionDigits: 0 }).format(amount);
...
const getProgressColor = (pct: number) => {
  ...
  if (pct >= 25)  return "from-orange-500 to-orange-600";
...
const AddPaymentModal = ({
  payment, isOpen, onClose, onSubmit, loading,
}: {
  ...
  useEffect(() => {
    if (payment) {
      setFormData((prev) => ({ ...prev, amount: payment.remainingAmount.toString() }));
    }
  }, [payment]);
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input type="number" value={formData.amount}
                ...
                placeholder="0.00" max={payment.remainingAmount}
                className="w-full pl-14 pr-4 py-3 border border-slate-200 rounded-xl
                  focus:outline-none focus:ring-2 focus:ring-emerald-500/20
                  focus:border-emerald-400 text-lg font-semibold" required />
            </div>
            <div className="flex gap-2 mt-2">
              {[
                { label: "50%",         val: Math.floor(payment.remainingAmount / 2).toString()       },
                { label: "75%",         val: Math.floor(payment.remainingAmount * 0.75).toString()    },
                { label: "Full Amount", val: payment.remainingAmount.toString(), highlight: true       },
              ].map((btn) => (
                <button key={btn.label} type="button"
                  onClick={() => setFormData({ ...formData, amount: btn.val })}
          ...
          <div className="grid grid-cols-2 gap-4">
              ...
              <div className="relative">
                <select value={formData.paymentMethod}
                  ...
                  className="w-full px-4 py-3 border border-slate-200 rounded-xl
                    focus:outline-none focus:ring-2 focus:ring-emerald-500/20
                    focus:border-emerald-400 appearance-none bg-white" required>
            ...
            <div>
              ...
              <input type="date" value={formData.paymentDate}
                ...
                className="w-full px-4 py-3 border border-slate-200 rounded-xl
                  focus:outline-none focus:ring-2 focus:ring-emerald-500/20
                  focus:border-emerald-400" required />
            </div>
          </div>
          ...
          <div>
            ...
            <input type="text" value={formData.transactionRef}
              ...
              className="w-full px-4 py-3 border border-slate-200 rounded-xl
                focus:outline-none focus:ring-2 focus:ring-emerald-500/20
                focus:border-emerald-400 font-mono" />
          </div>
          ...
          <div className="space-y-5">
            ...
            <div>
              <h3 className="text-sm font-semibold text-slate-800 mb-3 flex items-center gap-2">
                <Plane size={16} className="text-blue-500" /> Booking Details
              </h3>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={formData.bookingId}
                    ...
                    className="w-full px-4 py-3 border border-slate-200 rounded-xl
                      ...
                      focus:border-blue-400 font-mono uppercase" required />
                </div>
                <div>
            ...
            <div>
              ...
              <div className="relative">
                <select value={formData.agentId}
                  onChange={(e) => setFormData({ ...formData, agentId: e.target.value })}
                  className="w-full px-4 py-3 border border-slate-200 rounded-xl
                    focus:outline-none focus:ring-2 focus:ring-blue-500/20
                    focus:border-blue-400 appearance-none bg-white" required>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={formData.passengerName}
                  ...
                  className="w-full px-4 py-3 border border-slate-200 rounded-xl
                    focus:outline-none focus:ring-2 focus:ring-blue-500/20
                    focus:border-blue-400 uppercase" required />
              </div>
              <div>
            ...
            <div>
              <h3 className="text-sm font-semibold text-slate-800 mb-3 flex items-center gap-2">
                <Coins size={16} className="text-emerald-500" /> Payment Details
              </h3>
              <div className="grid grid-cols-2 gap-4">
                <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="relative">
                  <select value={formData.dueInterval}
            ...
            {totalAmount > 0 && (
              <div className="bg-blue-50 rounded-xl p-4 border border-blue-100">
                <h4 className="text-sm font-semibold text-blue-800 mb-3 flex items-center gap-2">
                  <ListChecks size={16} /> Payment Schedule Preview
                </h4>
...
const PaymentDetailModal = ({
  payment, onClose, onRecordPayment, onSendReminder,
}: {
  payment:         PartialPayment | null;
  onClose:         () => void;
  ...
  const progressPct       = (payment.paidAmount / payment.totalAmount) * 100;
    ...
    >
      ...
      >
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-blue-100 text-sm">
                  Booking: {payment.bookingId} • PNR: {payment.pnr}
                </p>
              </div>
            </div>
            <button onClick={onClose} className="p-2 hover:bg-white/20 rounded-xl transition">
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-280px)]">
          ...
          <div className="bg-slate-50 rounded-2xl p-5 mb-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-3xl font-bold text-slate-800">{progressPct.toFixed(0)}%</p>
              </div>
              <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full
                text-sm font-semibold ${statusConfig.bg} ${statusConfig.text}`}>
                {statusConfig.icon}
                {statusConfig.label}
              </span>
            </div>
            <div className="h-3 bg-slate-200 rounded-full overflow-hidden mb-4">
              <motion.div
                ...
                className={`h-full bg-gradient-to-r ${getProgressColor(progressPct)} rounded-full`}
              />
            </div>
            ...
            <div className={`rounded-xl p-4 mb-6 flex items-center gap-3
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Final due date: {formatDate(payment.dueDate)}
                </p>
              </div>
            </div>
          )}
                ...
                <div className="flex-1 flex items-center justify-center px-4">
                  <div className="flex items-center w-full">
                    ...
                    <div className="flex-1 h-px bg-slate-500 mx-2 relative">
                      <Plane size={16} className="text-white absolute left-1/2
                        -translate-x-1/2 -top-2 rotate-90" />
                    </div>
          ...
          <div className="mb-6">
            <h3 className="font-semibold text-slate-800 mb-3 flex items-center gap-2">
              <Building2 size={16} className="text-purple-500" /> Agent Information
            </h3>
          ...
          <div>
            <h3 className="font-semibold text-slate-800 mb-3 flex items-center gap-2">
              <Layers size={16} className="text-amber-500" /> Payment Installments
            </h3>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center
                          ${inst.status === "paid"    ? "bg-emerald-100" :
                            inst.status === "overdue" ? "bg-rose-100"    : "bg-slate-100"}`}>
                          {inst.status === "paid"
                      ...
                      <div className="text-right">
                        ...
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5
                          rounded-full text-xs font-semibold ${instStatus.bg} ${instStatus.text}`}>
                          {instStatus.icon}
                          {instStatus.label}
                        </span>
                      </div>
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex items-center
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => handleCopy(payment.id)}
              className="p-2 hover:bg-white rounded-lg transition" title="Copy ID">
              {copied
          ...
          <div className="flex items-center gap-2">
            {payment.status !== "completed" && payment.status !== "cancelled" && (
              <>
                <button onClick={() => onSendReminder(payment)}
                  className="px-4 py-2 bg-amber-100 text-amber-700 rounded-lg text-sm
                    font-medium hover:bg-amber-200 transition flex items-center gap-2">
                  <BellRing size={16} /> Send Reminder
                </button>
                <button onClick={() => onRecordPayment(payment)}
          ...
          <>
            ...
            >
              <button onClick={() => { onExport("pdf"); setIsOpen(false); }}
                ...
                <File size={16} className="text-rose-500" /> Export as PDF
              </button>
...
export default function PartialPaymentPage() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      total: payments.length, active, completed, overdue,
      totalOutstanding, totalCollected, totalExpected,
  ...
  const uniqueAgents = useMemo(() => {
    ...
    payments.forEach((p) => {
      if (!agentsMap.has(p.agentId)) agentsMap.set(p.agentId, { name: p.agentName, company: p.agentCompany });
    });
  ...
  const filteredData = useMemo(() => {
    ...
    let result = payments.filter((item) => {
      ...
      const matchFrom   = dateFrom ? new Date(item.createdAt) >= new Date(dateFrom) : true;
      const matchTo     = dateTo   ? new Date(item.createdAt) <= new Date(dateTo + "T23:59:59") : true;
      return matchSearch && matchStatus && matchAgent && matchFrom && matchTo;
    });
  ...
  const handleReset = () => {
    setSearchQuery(""); setStatusFilter(""); setAgentFilter("");
  ...
  const handleRecordPaymentSubmit = (data: Record<string, string>) => {
    setLoading(true);
    setTimeout(() => {
      if (paymentForRecording) {
        ...
        setPayments((prev) =>
          prev.map((p) => {
            if (p.id !== paymentForRecording.id) return p;
  ...
  const handleCreatePartialPayment = (data: Record<string, string>) => {
    setLoading(true);
    setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30
      ...
      <div className="max-w-[1800px] mx-auto space-y-6">
          ...
          <div className="flex items-center gap-2 md:gap-3 flex-wrap">
            ...
            <button onClick={handleRefresh} disabled={loading}
              className="flex items-center gap-2 px-4 py-2.5 bg-white border
                ...
                hover:bg-slate-50 transition shadow-sm disabled:opacity-70">
              <RefreshCw size={16} className={loading ? "animate-spin" : ""} />
              <span className="hidden sm:inline">Refresh</span>
            </button>
            <button onClick={() => setShowCreateModal(true)}
              className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r
                from-purple-600 to-indigo-600 text-white rounded-xl text-sm
                font-medium hover:from-purple-700 hover:to-indigo-700
                transition shadow-sm">
        ...
        >
          {[
            { label: "Total Plans",   value: stats.total,                              icon: <Layers        size={20} className="text-blue-600"    />, border: "border-slate-100",   bg: "bg-blue-50"    },
            { label: "Active",        value: stats.active,                             icon: <CircleDashed  size={20} className="text-blue-600"    />, border: "border-blue-100",    bg: "bg-blue-50",    text: "text-blue-600"    },
            { label: "Completed",     value: stats.completed,                          icon: <CircleCheck   size={20} className="text-emerald-600" />, border: "border-emerald-100", bg: "bg-emerald-50", text: "text-emerald-600" },
            { label: "Overdue",       value: stats.overdue,                            icon: <AlertTriangle size={20} className="text-rose-600"    />, border: "border-rose-100",    bg: "bg-rose-50",    text: "text-rose-600"    },
            { label: "Outstanding",   value: formatCurrency(stats.totalOutstanding),   icon: <Coins         size={20} className="text-amber-600"   />, border: "border-amber-100",   bg: "bg-amber-50",   text: "text-amber-600"   },
            { label: "Collected",     value: formatCurrency(stats.totalCollected),     icon: <HandCoins     size={20} className="text-purple-600"  />, border: "border-purple-100",  bg: "bg-purple-50",  text: "text-purple-600"  },
          ].map((stat, i) => (
            <div key={i} className={`bg-white rounded-2xl p-4 border ${stat.border} shadow-sm`}>
              <div className="flex items-center justify-between">
                <div>
          ...
          <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-4
            shadow-sm col-span-2 md:col-span-1">
            <div className="flex items-center justify-between">
              <div>
              ...
              <div className="relative">
                <select value={statusFilter}
                  ...
                  className="appearance-none pl-4 pr-10 py-3 border border-slate-200
                    ...
                    focus:ring-purple-500/20 focus:border-purple-400 bg-white
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gradient-to-r from-slate-50 to-slate-100
                    ...
                    {["Plan ID", "Booking", "Agent", "Progress", "Status", "Due Date", "Remaining", "Actions"].map((h) => (
                      <th key={h} className={`px-4 py-4 text-xs font-semibold text-slate-500
                        uppercase tracking-wider
                        ${h === "Remaining" ? "text-right" : h === "Actions" ? "text-center" : "text-left"}`}>
                        {h}
                      </th>
                    ))}
                  </tr>
                        ...
                        >
                          ...
                          <td className="px-4 py-4">
                            <button onClick={() => handleViewPayment(payment)}
                              className="inline-flex items-center gap-1.5 bg-purple-600
                                hover:bg-purple-700 text-white px-3 py-1.5 rounded-lg
                          ...
                          <td className="px-4 py-4">
                            <div className="flex items-center gap-3">
                              <div className="w-9 h-9 bg-gradient-to-br from-purple-500
                          ...
                          <td className="px-4 py-4 text-right">
                            <p className={`font-bold
                              ${payment.remainingAmount > 0 ? "text-rose-600" : "text-emerald-600"}`}>
                              {formatCurrency(payment.remainingAmount)}
                            </p>
                          </td>
                    ...
                    <tr>
                      <td colSpan={8} className="px-4 py-16 text-center">
                        ...
                        >
                          ...
                          <button onClick={handleReset}
                            className="mt-4 px-5 py-2.5 bg-purple-600 text-white
      ...
      <AnimatePresence>
        {showAddPaymentModal && paymentForRecording && (
          <AddPaymentModal
            ...
            onClose={() => { setShowAddPaymentModal(false); setPaymentForRecording(null); }}
