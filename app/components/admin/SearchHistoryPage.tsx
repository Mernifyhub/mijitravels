...
const generateSearchHistory = (): SearchEntry[] => {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    searchDate.setHours(searchDate.getHours() - Math.floor(Math.random() * 168)); // Last 7 days
    ...
    if (returnDate) {
      returnDate.setDate(returnDate.getDate() + Math.floor(Math.random() * 14) + 3);
    }
...
const getCabinClassConfig = (cabinClass: string) => {
    ...
    >
      ...
      >
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
          ...
          <div className="bg-slate-50 rounded-xl p-4 mb-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="text-right">
                <code className="px-2 py-1 bg-slate-200 rounded text-xs font-mono">{search.agentId}</code>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6">
            ...
            <div className="bg-slate-50 rounded-xl p-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-slate-800">{(search.searchTime / 1000).toFixed(2)}s</span>
              </div>
            </div>
            <div className="bg-slate-50 rounded-xl p-3">
          ...
          {/* Conversion Status */}
          <div className={`rounded-xl p-4 ${search.converted ? "bg-emerald-50 border border-emerald-200" : "bg-slate-50"}`}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className={`font-semibold ${search.converted ? "text-emerald-700" : "text-slate-700"}`}>
                    {search.converted ? "Converted to Booking" : "Not Converted"}
                  </p>
                  <p className="text-sm text-slate-500">
                    {search.converted ? "This search resulted in a booking" : "No booking was made from this search"}
                  </p>
                </div>
              </div>
              {search.bookingId && (
...
const AnalyticsCard = ({
  ...
  >
    <div className="flex items-start justify-between">
      ...
      <div className={`w-12 h-12 ${color.replace("text-", "bg-").replace("-600", "-50").replace("-700", "-50")} rounded-xl flex items-center justify-center`}>
        {icon}
      </div>
    </div>
  </motion.div>
  ...
  >
    ...
    <div className="space-y-3">
      {agents.slice(0, 5).map((agent, index) => (
        <div key={agent.id} className="flex items-center justify-between p-2 hover:bg-slate-50 rounded-xl transition">
          <div className="flex items-center gap-3">
...
export default function SearchHistoryPage() {
  ...
  const stats = useMemo(() => {
    ...
    const todaySearches = searchHistory.filter(
      (s) => new Date(s.searchedAt).toDateString() === today
    );
    ...
    const avgSearchTime = searchHistory.reduce((sum, s) => sum + s.searchTime, 0) / totalSearches;
  ...
  const topAgents = useMemo(() => {
    ...
    searchHistory.forEach((s) => {
      if (!agentStats[s.agentId]) {
        agentStats[s.agentId] = {
          name: s.agentName,
          company: s.agentCompany,
          searches: 0,
          conversions: 0,
        };
      }
  ...
  const uniqueAgents = useMemo(() => {
    ...
    searchHistory.forEach((s) => {
      if (!agents.has(s.agentId)) {
        agents.set(s.agentId, { name: s.agentName, company: s.agentCompany });
      }
    });
  ...
  const filteredData = useMemo(() => {
    ...
    let result = searchHistory.filter((item) => {
      const matchSearch =
        query === ""
          ...
          : item.id.toLowerCase().includes(query) ||
            ...
            item.agentCompany.toLowerCase().includes(query) ||
            item.origin.toLowerCase().includes(query) ||
            item.destination.toLowerCase().includes(query) ||
            item.originCity.toLowerCase().includes(query) ||
            item.destinationCity.toLowerCase().includes(query);
      ...
      if (dateFrom) {
        matchDateFrom = new Date(item.searchedAt) >= new Date(dateFrom);
      }
      if (dateTo) {
        matchDateTo = new Date(item.searchedAt) <= new Date(dateTo + "T23:59:59");
      }

      return matchSearch && matchAgent && matchTripType && matchConverted && matchDateFrom && matchDateTo;
    });
      ...
      <div className="max-w-[1800px] mx-auto space-y-6">
        ...
        >
            ...
            <div className="flex items-center gap-2 md:gap-3 flex-wrap">
              ...
              <div className="relative">
                <select
                  ...
                  className="appearance-none pl-4 pr-10 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 bg-white cursor-pointer min-w-[150px]"
                >
          ...
          <AnimatePresence>
            {showFilters && (
              ...
              >
                <div className="mt-4 pt-4 border-t border-slate-100 grid grid-cols-2 md:grid-cols-5 gap-3 md:gap-4">
                  <div>
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 bg-white appearance-none pr-10"
                      >
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-500 mb-1.5">Converted</label>
                    <div className="relative">
                      <select
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <p className="text-sm text-slate-500">
            ...
            <span className="font-semibold text-slate-700">{filteredData.length}</span> searches
          </p>
                    ...
                    paginatedData.map((search, index) => {
                      ...
                      return (
                        ...
                        >
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <div className="flex items-center gap-1.5">
                                  <p className="text-sm font-medium text-slate-800">{search.agentName}</p>
                                  {search.verified && <Verified size={12} className="text-blue-500" />}
                                </div>
                          ...
                          {/* Type */}
                          <td className="px-4 py-4">
                            <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-semibold ${tripConfig.bg} ${tripConfig.text}`}>
                              {tripConfig.icon}
                              {tripConfig.label}
                            </span>
                          </td>
                          ...
                          <td className="px-4 py-4">
                            ...
                            <p className="text-xs text-slate-400 mt-0.5">
                              {search.adults}A {search.children > 0 && `${search.children}C `}{search.infants > 0 && `${search.infants}I`}
                            </p>
                          </td>
                          ...
                          <td className="px-4 py-4">
                            <div className="flex items-center gap-2">
                              <span className="text-sm font-semibold text-slate-800">{search.resultsCount}</span>
                              <span className="text-xs text-slate-400">flights</span>
                            </div>
                            <p className="text-xs text-slate-400">{(search.searchTime / 1000).toFixed(2)}s</p>
                          </td>
                          ...
                          <td className="px-4 py-4">
                            {search.converted ? (
                              <div>
                                ...
                                {search.bookingId && (
                                  <p className="text-xs text-emerald-600 mt-1 font-mono">{search.bookingId}</p>
                                )}
                              </div>
                            ) : (
