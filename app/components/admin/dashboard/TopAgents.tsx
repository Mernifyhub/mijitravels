// app/components/admin/dashboard/TopAgents.tsx
"use client";
...
export default function TopAgents({ agents, role }: TopAgentsProps) {
  const router = useRouter();
  const allAgentsRoute = role === "admin" ? "/admin/agent/all-agent" : "/manager/agent/all-agent";
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-sm">
      <div className="flex items-center justify-between mb-5">
        ...
        <button onClick={() => router.push(allAgentsRoute)} className="text-sm text-[#021f3b] font-medium hover:underline flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        {agents.length > 0 ? agents.map((agent: any, index: number) => {
          ...
          const initials = agent.avatar || agent.name.split(" ").map((n: string) => n[0]).join("");

          return (
            <div key={index} className={`relative p-4 rounded-xl border-2 transition-all cursor-pointer hover:shadow-md ${style.card}`}>
              <div className={`absolute -top-2 -right-2 w-8 h-8 rounded-full flex items-center justify-center shadow-md ${style.badge}`}>
                <span className="text-white text-xs font-bold">{index + 1}</span>
              </div>
              <div className={`w-12 h-12 mx-auto rounded-full flex items-center justify-center text-white font-bold text-sm mb-3 ${style.avatar}`}>
                {initials}
              </div>
