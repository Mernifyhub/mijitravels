// app/components/manager/dashboard/RecentDeposits.tsx
"use client";
...
export default function RecentDeposits({ deposits }: RecentDepositsProps) {
  ...
  const getDepositIcon = (status: string) => {
    switch (status) {
      case "approved":
  ...
  const getDepositBg = (status: string) => {
    switch (status) {
      case "approved":
        return "bg-green-100";
      case "pending":
      ...
      <div className="space-y-4">
        {deposits.length > 0 ? (
          deposits.map((deposit: any) => (
            ...
            >
              <div
                className={`w-10 h-10 rounded-full flex items-center justify-center ${getDepositBg(deposit.status)}`}
              >
                {getDepositIcon(deposit.status)}
              </div>
