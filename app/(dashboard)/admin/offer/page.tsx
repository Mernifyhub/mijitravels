...
export default function OfferPage() {
  ...
  const handleSave = () => {
    ...
    if (editMode && selectedOffer) {
      setOffers(offers.map((o) => (o.id === selectedOffer.id ? { ...o, ...formData } : o)));
  ...
  const handleBulkDelete = () => {
    if (confirm(`Delete ${selectedItems.length} offers?`)) {
      setOffers(offers.filter((o) => !selectedItems.includes(o.id)));
      setSelectedItems([]);
    ...
    if (search) {
      ...
      result = result.filter(
        (o) =>
          ...
          o.couponCode?.toLowerCase().includes(searchLower)
      );
    }
        ...
        offers.length > 0
          ? (
              (offers.reduce((sum, o) => sum + o.conversions, 0) /
                Math.max(offers.reduce((sum, o) => sum + o.clicks, 0), 1)) *
              100
            ).toFixed(1)
          : "0",
    }),
  ...
  // Configs
  const typeConfig = {
    banner: { color: "bg-blue-100 text-blue-700 border-blue-200", icon: <ImageIcon size={14} /> },
    popup: { color: "bg-purple-100 text-purple-700 border-purple-200", icon: <Zap size={14} /> },
    slider: { color: "bg-green-100 text-green-700 border-green-200", icon: <RefreshCw size={14} /> },
    promo: { color: "bg-orange-100 text-orange-700 border-orange-200", icon: <Tag size={14} /> },
  };
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Manage promotional banners and offers</p>
            </div>
          </div>
            ...
            <table className="w-full">
              <thead className="bg-gradient-to-r from-[#021f3b] to-[#0a3a6b] text-white">
                <tr>
                  ...
                  <th className="px-4 py-4 text-center text-xs font-semibold uppercase tracking-wider">Discount</th>
                  ...
                  <th className="px-4 py-4 text-center text-xs font-semibold uppercase tracking-wider">Duration</th>
                  ...
                  <tr>
                    <td colSpan={10} className="px-4 py-12 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-gray-400 text-sm">Create your first promotional offer</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                    ...
                    >
                      <td className="px-4 py-4">
                        <input
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              setSelectedItems([...selectedItems, offer.id]);
                            } else {
                              setSelectedItems(selectedItems.filter((id) => id !== offer.id));
                            }
                          }}
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-16 h-10 bg-gray-100 rounded-lg overflow-hidden relative flex-shrink-0">
                          ...
                          <div>
                            ...
                            {offer.couponCode && (
                              <span className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 bg-yellow-100 text-yellow-700 rounded text-xs font-mono">
                                <Tag size={10} />
                                {offer.couponCode}
                              </span>
                            )}
                      ...
                      <td className="px-4 py-4 text-center">
                        <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium border ${typeConfig[offer.type].color}`}>
                      ...
                      <td className="px-4 py-4 text-center">
                        <button
                          ...
                          className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium ${statusConfig[offer.status].color}`}
                        >
        ...
        <div>
          {paginatedData.length === 0 ? (
            <div className="bg-white p-16 rounded-xl text-center shadow-sm border">
              ...
              <p className="text-gray-500 font-medium">No offers found</p>
              <p className="text-gray-400 text-sm mt-1">Create your first promotional offer</p>
            </div>
          ) : (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {paginatedData.map((offer) => (
                ...
                >
                  {/* Image */}
                  <div className="relative h-40 bg-gradient-to-br from-orange-400 via-pink-500 to-purple-500">
                    ...
                    <div className="absolute top-3 left-3 flex items-center gap-2">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusConfig[offer.status].color}`}>
                        {offer.status}
                      </span>
                      {offer.discount && (
                  ...
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <h3 className="font-semibold text-gray-800 line-clamp-1">{offer.title}</h3>
                      <span className={`flex-shrink-0 px-2 py-0.5 rounded text-xs font-medium ${priorityConfig[offer.priority]}`}>
                        {offer.priority}
                      </span>
                    </div>
                    ...
                    {offer.couponCode && (
                      <div className="flex items-center gap-2 mb-3">
                        <span className="flex-1 px-3 py-1.5 bg-yellow-50 border border-yellow-200 rounded-lg text-center font-mono text-sm text-yellow-700">
                          {offer.couponCode}
                        </span>
                        <button className="p-1.5 bg-gray-100 rounded-lg text-gray-500 hover:bg-gray-200 transition">
                          <Copy size={14} />
                        </button>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-3 gap-2 pt-3 border-t border-gray-100">
                      <div className="text-center">
                        <p className="text-lg font-bold text-gray-800">{offer.views.toLocaleString()}</p>
                        <p className="text-xs text-gray-500">Views</p>
                      </div>
                      <div className="text-center border-x border-gray-100">
                        <p className="text-lg font-bold text-gray-800">{offer.clicks.toLocaleString()}</p>
            ...
            <div className="mt-6 flex justify-center">
              <div className="flex items-center gap-2">
                <button
                  onClick={() => setCurrentPage(currentPage - 1)}
                  disabled={currentPage === 1}
                  className="px-4 py-2 rounded-lg border border-gray-200 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition flex items-center gap-1"
                >
                  <ChevronLeft size={16} />
                  Previous
                </button>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-gray-800">{editMode ? "Edit Offer" : "Create Offer"}</h2>
                  ...
                  <label htmlFor="image-upload" className="cursor-pointer">
                    {imagePreview ? (
                      ...
                      <>
                        ...
                        <p className="text-xs text-gray-400 mt-1">Recommended: 800×370px, max 300KB</p>
                      </>
                    )}
                  </label>
                </div>
      ...
      {showViewModal && selectedOffer && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white">
              <div className="flex items-center gap-3">
                <div className={`p-2 rounded-xl border ${typeConfig[selectedOffer.type].color}`}>
                  {typeConfig[selectedOffer.type].icon}
                </div>
                <div>
                  <h2 className="text-lg font-bold text-gray-800">{selectedOffer.title}</h2>
                  <p className="text-sm text-gray-500">Created by {selectedOffer.createdBy}</p>
                </div>
              </div>
            ...
            <div className="p-6">
              {/* Banner Preview */}
              <div className="relative h-48 bg-gradient-to-br from-orange-400 via-pink-500 to-purple-500 rounded-xl mb-6 flex items-center justify-center">
                <ImageIcon size={48} className="text-white/50" />
                {selectedOffer.discount && (
                  <div className="absolute top-4 right-4 px-4 py-2 bg-green-500 text-white rounded-full font-bold shadow-lg">
              ...
              <div className="flex flex-wrap items-center gap-2 mb-6">
                <span className={`inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-sm font-medium border ${typeConfig[selectedOffer.type].color}`}>
                  {typeConfig[selectedOffer.type].icon}
                  {selectedOffer.type}
                </span>
                <span className={`inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-sm font-medium ${statusConfig[selectedOffer.status].color}`}>
                  {statusConfig[selectedOffer.status].icon}
                  {selectedOffer.status}
                </span>
                <span className={`px-3 py-1.5 rounded-full text-sm font-medium ${priorityConfig[selectedOffer.priority]}`}>
              ...
              {selectedOffer.couponCode && (
                <div className="flex items-center gap-3 mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-xl">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="font-mono font-bold text-lg text-yellow-800">{selectedOffer.couponCode}</p>
                  </div>
                  <button className="p-2 bg-yellow-200 rounded-lg text-yellow-700 hover:bg-yellow-300 transition">
                    <Copy size={16} />
                  </button>
                </div>
              )}
              ...
              <div className="grid grid-cols-4 gap-4 mb-6">
                <div className="bg-blue-50 rounded-xl p-4 text-center">
                  <p className="text-2xl font-bold text-blue-600">{selectedOffer.views.toLocaleString()}</p>
                  <p className="text-xs text-blue-700">Views</p>
                </div>
                <div className="bg-green-50 rounded-xl p-4 text-center">
                  <p className="text-2xl font-bold text-green-600">{selectedOffer.clicks.toLocaleString()}</p>
                  <p className="text-xs text-green-700">Clicks</p>
                </div>
                <div className="bg-purple-50 rounded-xl p-4 text-center">
                  <p className="text-2xl font-bold text-purple-600">{selectedOffer.conversions}</p>
                  <p className="text-xs text-purple-700">Conversions</p>
                </div>
                <div className="bg-orange-50 rounded-xl p-4 text-center">
                  <p className="text-2xl font-bold text-orange-600">
                    {selectedOffer.clicks > 0 ? ((selectedOffer.conversions / selectedOffer.clicks) * 100).toFixed(1) : 0}%
                  </p>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <p className="text-sm font-medium text-gray-700">
                    {new Date(selectedOffer.startDate).toLocaleDateString()} - {new Date(selectedOffer.endDate).toLocaleDateString()}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 border-t border-gray-100 flex items-center justify-between sticky bottom-0 bg-white">
              ...
              >
                {selectedOffer.status === "active" ? <ToggleLeft size={14} /> : <ToggleRight size={14} />}
